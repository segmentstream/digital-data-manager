...
const addEvent = (targetScrollDepth, handler) => {
  events.push({ targetScrollDepth, handler, isFired: false })
}

const processEvents = () => {
  const scrolledPercent = getScrollPercent()
  events = events.map((event) => {
    if (!event.isFired && event.targetScrollDepth <= scrolledPercent) {
      event.handler(event.targetScrollDepth)
