    ...
    describe('before loading', () => {
      ...
      describe('#initialize', () => {
        it('should require "linkid.js" if enhanced link attribution is `true`', () => {
          ...
          assert.strict.deepEqual(argumentsToArray(window.ga.q[1]), ['require', 'linkid', 'linkid.js'])
        })

        it('should create window.GoogleAnalyticsObject', () => {
          assert.ok(!window.GoogleAnalyticsObject)
          ddManager.initialize()
          assert.strict.equal(window.GoogleAnalyticsObject, 'ga')
        })

        it('should create window.ga', () => {
          assert.ok(!window.ga)
          ddManager.initialize()
          assert.strict.equal(typeof window.ga, 'function')
        })

        it('should create window.ga.l', () => {
          assert.ok(!window.ga)
          ddManager.initialize()
          assert.strict.equal(typeof window.ga.l, 'number')
        })

        it('should call window.ga.create with options', () => {
          ddManager.initialize()
          assert.strict.deepEqual(argumentsToArray(window.ga.q[0]), ['create', options.trackingId, {
            cookieDomain: 'none',
        ...
        it('should anonymize the ip', () => {
          ddManager.initialize()
          assert.strict.deepEqual(argumentsToArray(window.ga.q[1]), ['set', 'anonymizeIp', true])
        })
